...
# Copy Makefile so the script can install tools if needed
cp "${REPO_ROOT}/Makefile" "${TMPDIR}/Makefile"

# ── run the script ────────────────────────────────────────────────────────────

readonly TEST_VERSION="7.7.7"
readonly TEST_REGISTRY="test.registry.io/test-namespace"
readonly TEST_CHANNEL="stable-v7.7"

echo ""
echo "Running propagate-version.bash ${TEST_VERSION} ${TEST_REGISTRY} ${TEST_CHANNEL} ..."
...
assert_eq "spec.image" \
	"${TEST_REGISTRY}/spyre-operator-catalog:${TEST_VERSION}" \
