...
if [ "x" == "x${NAMESPACE}" ]; then
	echo "Error: NAMESPACE needs to be supplied as the third argument."
	exit 1
fi
...
function image_name() {
	local component=${1}
	local image_name=$(${YQ} -r .${component}.image ${BOM})
	echo ${image_name}
}
function propagate_version() {
	...
	${YQ} eval -i ".spec.image=\"${REGISTRY}/${NAMESPACE}/spyre-operator-catalog:${VERSION}\"" ${REPO_ROOT}/config/olm/catalog-source.yaml
	...
	#patch power cluster service with versions
	${YQ} eval -i ".spec.metricsExporter.version=\"$(image_tag exporter)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.ppc64le.yaml
	${YQ} eval -i ".spec.devicePlugin.version=\"$(image_tag devicePlugin)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.ppc64le.yaml
	${YQ} eval -i ".spec.devicePlugin.initContainer.version=\"$(image_tag devicePluginInit)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.ppc64le.yaml
	${YQ} eval -i ".spec.scheduler.version=\"$(image_tag scheduler)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.ppc64le.yaml
	${YQ} eval -i ".spec.podValidator.version=\"$(image_tag podValidator)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.ppc64le.yaml
	${YQ} eval -i ".spec.healthChecker.version=\"$(image_tag healthChecker)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.ppc64le.yaml

	#patch s390x cluster service with versions
	${YQ} eval -i ".spec.metricsExporter.version=\"$(image_tag exporter)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.s390x.yaml
	${YQ} eval -i ".spec.devicePlugin.version=\"$(image_tag devicePlugin)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.s390x.yaml
	${YQ} eval -i ".spec.devicePlugin.initContainer.version=\"$(image_tag devicePluginInit)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.s390x.yaml
	${YQ} eval -i ".spec.scheduler.version=\"$(image_tag scheduler)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.s390x.yaml
	${YQ} eval -i ".spec.podValidator.version=\"$(image_tag podValidator)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.s390x.yaml
	${YQ} eval -i ".spec.healthChecker.version=\"$(image_tag healthChecker)\"" ${REPO_ROOT}/config/samples/spyre_v1alpha1_spyreclusterpolicy.s390x.yaml
