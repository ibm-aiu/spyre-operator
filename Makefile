...
test: fmt vet ginkgo jq manifests generate envtest ## Run unit tests.
	...
	KUBEBUILDER_ASSETS="$(shell $(ENVTEST) use $(ENVTEST_K8S_VERSION) -p path)" $(LOCALBIN)/ginkgo run --label-filter="!(e2e||integration)" --seed 777 --cover --coverprofile=$(COVERAGE_FILE) ./api/... ./controllers/... ./pkg/... ./internal/...
